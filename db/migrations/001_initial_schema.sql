...
create table if not exists app_users (
  id uuid primary key default gen_random_uuid(), wallet_address text not null unique,
  display_name text not null, role text not null default 'Member' check (role in ('Member','Admin','Support')),
  referral_code text not null unique, avatar_url text, created_at timestamptz not null default now(), updated_at timestamptz not null default now()
);
...
create table if not exists user_sessions (
  id uuid primary key, user_id uuid not null references app_users(id) on delete cascade, wallet_address text not null,
  expires_at timestamptz not null, revoked_at timestamptz, ip_address inet, user_agent text, created_at timestamptz not null default now()
);
...
create table if not exists programs (
  id uuid primary key default gen_random_uuid(), code text not null unique, name text not null, levels integer not null,
  capacity integer not null, description text, active boolean not null default true, sort_order integer not null default 0,
...
create table if not exists matrix_nodes (
  ...
  user_id uuid references app_users(id) on delete set null, referrer_user_id uuid references app_users(id) on delete set null,
  activated_at timestamptz, completed_at timestamptz, created_at timestamptz not null default now(),
  unique(program_id,position)
);
create table if not exists matrix_memberships (
  id uuid primary key default gen_random_uuid(), user_id uuid not null references app_users(id) on delete cascade,
  program_id uuid not null references programs(id) on delete cascade, node_id uuid references matrix_nodes(id) on delete set null,
  referrer_user_id uuid references app_users(id) on delete set null, level integer not null, position integer not null,
...
create table if not exists audit_logs (
  id uuid primary key default gen_random_uuid(), actor_user_id uuid references app_users(id) on delete set null,
...
create index if not exists idx_matrix_nodes_program on matrix_nodes(program_id,level,position);
create index if not exists idx_matrix_memberships_referrer on matrix_memberships(referrer_user_id,created_at);
create index if not exists idx_ledger_user_time on ledger_transactions(user_id,occurred_at desc);
...
insert into programs(code,name,levels,capacity,description,sort_order) values
