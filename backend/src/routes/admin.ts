...
router.get('/summary',async(_req,res,next)=>{try{const [users,txs,pending]=await Promise.all([query(`select count(*)::int count from app_users`),query(`select count(*)::int count from ledger_transactions`),query(`select count(*)::int count from withdrawal_requests where status='pending'`)]);res.json({users:users.rows[0]?.count??0,transactions:txs.rows[0]?.count??0,pendingWithdrawals:pending.rows[0]?.count??0});}catch(e){next(e)}});
